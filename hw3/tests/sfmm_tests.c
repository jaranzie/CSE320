...
/* Malloc a page, then free it*/
Test(sfmm_student_suite, student_test_1, .timeout = TEST_TIMEOUT) {
    sf_errno = 0;
    void *x = sf_malloc(1968);
    cr_assert_not_null(x, "x is NULL!");
    assert_free_block_count(0, 0);
    cr_assert(sf_errno == 0, "sf_errno is not 0!");
    sf_free(x);
    assert_free_block_count(1984, 1);
    assert_free_list_size(7,1); /* make sure it gets added back as wilderness*/
}

/* Malloc a page, malloc again, and ensure that extra space is wilderness*/
...
/* TEST MEMALIGN */
Test(sfmm_student_suite, student_test_3) {
    sf_errno = 0;
    int align = 64;
    int *x = sf_memalign(64, align);
    cr_assert(((((size_t)x) & (~(align-1))) == (size_t)x));
    sf_free(x);
}

/*MEM ALIGN ON BOUNDARY*/
Test(sfmm_student_suite, student_test_6) {
    sf_errno = 0;
    int align = 64;
    void* y = sf_malloc(2);
...
Test(sfmm_student_suite, student_test_4, .timeout = TEST_TIMEOUT) {
    sf_errno = 0;
    int *x = sf_memalign(1000, 3); /* Not power of 2 */
    cr_assert(x == NULL);
...
Test(sfmm_student_suite, student_test_5, .signal = SIGABRT, .timeout = TEST_TIMEOUT) {
    sf_errno = 0;
    int *x = sf_malloc(64); /* Not power of 2 */
    sf_free(x - 11);
}